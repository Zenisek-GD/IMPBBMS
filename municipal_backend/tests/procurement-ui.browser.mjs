...
test("procurement screens support dynamic evaluation, schedule approval, BAC failure review and reports", {timeout:180000}, async (t) => {
  ...
  let role="twgMember",declared=false,declarationWrites=0,scheduleApprovalWrites=0,failureStatus="draft";
  ...
  const evaluationPlan={status:"approved",approvalReference:"BAC-QA-001",preparedById:8,qualityWeight:75,financialWeight:25,passingScore:60,criteria:[{key:"experience",name:"Experience",description:"Relevant completed projects",maxScore:100,weight:100}]};
  const complianceRequirements=[{key:"eligibility",label:"Eligibility documents"},{key:"specifications",label:"Technical specifications"},{key:"security",label:"Bid security"}];
  ...
  page.on("request",async(request)=>{
    ...
    else if(key==="/bidding/rfqs") data=url.searchParams.has("page")?{rows:[rfq],total:1,page:1,pageSize:25,totalPages:1}:[rfq];
    else if(key==="/bidding/rfqs/1/bids") data={blind:true,qualityWeight:75,financialWeight:25,evaluationPlan:rfq.category==="consulting"?evaluationPlan:null,complianceRequirements,bids:[{id:1,blindLabel:"Bidder A",vendorName:null,status:"opened",totalBidPrice:null,evaluationCount:0,evaluations:[]}]};
    ...
    else if(key==="/bidding/rfqs/1/evaluator-declaration") {assert.equal(JSON.parse(request.postData()).declared,true);declared=true;declarationWrites++;data={message:"Declaration recorded. You may now prepare the technical assessment."};}
    else if(key==="/bidding/rfqs/1/evaluation-administration") data={declaration:declared?{noConflictDeclared:true,declaredAt:new Date().toISOString()}:null,conflicts:[],returns:[]};
    else if(key==="/bidding/rfqs/1/evaluation-plan") data={plan:evaluationPlan,amendments:[],canAmend:false};
    else if(key==="/bidding/rfqs/1/schedule/approve") {scheduleApprovalWrites++;rfq.scheduleApprovedAt=new Date().toISOString();data={message:"The procurement schedule was approved."};}
    else if(key==="/bidding/rfqs/1/schedule") data={...rfq,rfqId:rfq.id,locked:rfq.status!=="draft",approvedAt:rfq.scheduleApprovedAt??null,preparedById:8,amendments:[]};
    else if(key==="/bidding/rfqs/1/attempts") data={attempts:[{id:1,rfqId:1,rfqStatus:rfq.status,attemptNumber:1,status:rfq.status==="failed"?"failed":"ongoing",referenceNo:rfq.referenceNo,title:rfq.title,startedAt:new Date().toISOString(),failureRecords:[{id:1,failureNumber:"2026-QA-001",status:failureStatus,reason:"No responsive bids",category:"noResponsiveBids",explanation:"All submitted bids failed mandatory checks.",createdById:8,createdAt:new Date().toISOString(),supportingDocuments:[],votes:[]}]}],policy,eligibility:{eligible:false,missing:["Two approved failed attempts are required."]}};
  ...
  assert.equal(declarationWrites,1);await click("TWG assessment");await page.waitForFunction(()=>document.body.innerText.includes("TWG technical assessment"));assert.equal(await page.$('[role="dialog"]'),null,"TWG assessment is a full-page form");
  ...
  await page.select('[role="dialog"] select',"1");assert.match(await text(),/Attendance quorum is satisfied/);await click("Cancel");
  await click("Evaluate Quality and Price");await page.waitForFunction(()=>document.body.innerText.includes("Approved quality criteria"));
  assert.equal(await page.$('[role="dialog"]'),null,"consulting evaluation is a full-page form");
  assert.match(await text(),/Approved quality weight: 75%/);assert.match(await text(),/Experience/);assert.equal(await disabled("Submit final evaluation"),true);await click("Cancel");
  for(const category of ["goods","infrastructure"]){
    rfq.category=category;await page.goto(origin+"/evaluation",{waitUntil:"networkidle0"});await click("Evaluate compliance");
  ...
  rfq.status="draft";rfq.category="consulting";rfq.prebidRequired=false;
  await page.goto(origin+"/secretariat/rfq",{waitUntil:"networkidle0"});await click("Schedule / criteria");
  await page.waitForFunction(()=>document.body.innerText.includes("Approve official schedule"));
  assert.equal(await page.$('[role="dialog"]'),null,"schedule preparation is a full-page form");
  assert.match(await text(),/Schedule approval pending/);assert.doesNotMatch(await text(),/Save draft schedule/);
  await click("Approve official schedule");await page.waitForFunction(()=>document.body.innerText.includes("The procurement schedule was approved."));assert.equal(scheduleApprovalWrites,1);
  await click("Close");
  role="bacSecretariat";rfq.status="closed";rfq.category="goods";
  await page.goto(origin+"/secretariat/rfq",{waitUntil:"networkidle0"});await click("History / next action");
  await page.waitForFunction(()=>document.body.innerText.includes("Edit draft failure documents"));
  assert.match(await text(),/Draft Failure/);assert.match(await text(),/Submit to BAC/);assert.doesNotMatch(await text(),/Finalize BAC failure decision/);assert.doesNotMatch(await text(),/Review for Negotiated Procurement/);
  await page.keyboard.press("Escape");
  role="bacChairperson";failureStatus="submitted";
  await page.goto(origin+"/secretariat/rfq",{waitUntil:"networkidle0"});await click("History / next action");
  await page.waitForFunction(()=>document.body.innerText.includes("BAC failure review"));
  assert.match(await text(),/Waiting for BAC Review/);assert.doesNotMatch(await text(),/Finalize BAC failure decision/);await click("BAC failure review");
  await page.waitForFunction(()=>document.body.innerText.includes("BAC review findings"));assert.match(await text(),/Each official records approval, rejection or abstention using their own account/);await page.keyboard.press("Escape");
