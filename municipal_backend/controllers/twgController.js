...
import { ensureProcurementAttempt, assertNoPendingFailure } from "../services/procurementGovernance.js";
...
export const serializeAssessment = (row) => ({
  ...
  supportingInformation: row.supportingInformation, status: row.excludedForConflict ? "recused" : row.status,
...
export const listTwg = async (req, res) => {
  ...
  const declaration = await EvaluatorDeclaration.findOne({ where: { rfqId: rfq.id, userId: req.currentUser.id } }) ?? await TwgDeclaration.findOne({ where: { rfqId: rfq.id, memberId: req.currentUser.id } });
  const assessments = await TwgAssessment.findAll({
    where: req.permissions.has("bidding.chairEvaluation") ? {} : { [Op.or]: [{ status: "submitted" }, { memberId: req.currentUser.id }] },
...
export const declareTwgConflict = async (req, res) => {
  ...
  res.status(201).json({ noConflictDeclared: declaration.noConflictDeclared, declaredAt: declaration.declaredAt, message: declaration.noConflictDeclared ? "Conflict-of-interest declaration recorded. You may now prepare the TWG technical assessment." : "Conflict of interest recorded. Participation is blocked and reassignment is required." });
};
...
export const saveTwg = async (req, res) => {
  ...
  const assessment = await withAuditTransaction(async (transaction, audit) => {
    ...
    const declaration = await EvaluatorDeclaration.findOne({ where: { rfqId: rfq.id, userId: req.currentUser.id }, transaction }) ?? await TwgDeclaration.findOne({ where: { rfqId: rfq.id, memberId: req.currentUser.id }, transaction });
    if (!declaration?.noConflictDeclared) throw workflowError("Complete your no-conflict declaration before participating. A reported conflict requires reassignment.", 403);
    ...
    if (!row) await audit(actorAudit(req, { actionType: "evaluation.started", entityRef: "bid", entityId: bid.id, summary: "Authorized TWG evaluator started the technical assessment.", afterState: { rfqId: rfq.id, attemptId: attempt.id, attemptNumber: attempt.attemptNumber, evaluatorId: req.currentUser.id } }));
    row = row ? await row.update(values, { transaction }) : await TwgAssessment.create(values, { transaction });
    if (final) {
      const correction = await EvaluationReturn.findOne({ where: { targetType: "twgAssessment", targetId: row.id, correctedAt: null }, transaction });
      if (correction) await correction.update({ correctedAt: new Date(), updatedSubmission: row.toJSON() }, { transaction });
    }
    await audit(actorAudit(req, { actionType: final ? "twg.submitted" : "twg.draftSaved", entityRef: "twgAssessment", entityId: row.id, summary: final ? "TWG technical assessment and recommendation submitted for BAC review." : "TWG technical assessment saved as draft.", beforeState: before, afterState: { ...serializeAssessment(row), rfqId: rfq.id, attemptId: attempt.id, attemptNumber: attempt.attemptNumber } }));
    return row;
  });
