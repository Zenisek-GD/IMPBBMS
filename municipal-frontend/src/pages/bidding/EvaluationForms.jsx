    ...
    <form id="bid-evaluation-form" onSubmit={async (event) => {
      ...
      <LargeFormPage.Section title="Final decision and recommendation">
        <div className="space-y-3">
          ...
          {documents.length > 0 && <fieldset className="space-y-2"><legend className="text-sm">Supporting procurement documents</legend>{documents.map((document) => <label key={document.id} className="flex min-w-0 items-center gap-2 text-xs"><input type="checkbox" checked={supportingDocuments.some((item) => item.documentId === document.id)} onChange={(event) => setSupportingDocuments(event.target.checked ? [...supportingDocuments, { documentId: document.id }] : supportingDocuments.filter((item) => item.documentId !== document.id))} /><span className="min-w-0 break-all">{document.filename}</span></label>)}</fieldset>}
        </div>
      </LargeFormPage.Section>
